"""
Mathtermind Logging Module

This module provides a unified logging interface for the Mathtermind application.
It configures loggers for different components with appropriate handlers and formatters.
"""
...
DETAILED_LOG_FORMAT = "%(asctime)s | %(levelname)-8s | %(name)s | %(filename)s:%(lineno)d | %(message)s"
...
class LoggerManager:
    ...
    def __init__(self, log_dir: str = DEFAULT_LOG_DIR, log_file: str = DEFAULT_LOG_FILE):
        ...
        self.root_logger.setLevel(logging.DEBUG)  # Set root to DEBUG to allow all logs through
    ...
    def get_logger(self, name: str, 
                  ...
                  add_component_file: bool = False) -> logging.Logger:
        """
        ...
        logger.setLevel(logging.DEBUG)  # Allow all logs through, handlers will filter
    ...
    def add_daily_rotating_handler(self, logger_name: str, filename: str = None) -> None:
        """
...
# Convenience functions to get loggers for different components
def get_app_logger():
