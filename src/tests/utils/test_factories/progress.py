"""
Progress factories for Mathtermind tests.
"""
...
from src.db.models.content import (
    TheoryContent, ExerciseContent, 
    AssessmentContent, InteractiveContent
)
...
from src.tests.utils.test_factories.content import CourseFactory, ContentFactory
...
class ProgressFactory(BaseFactory[Progress]):
    ...
    def _get_defaults(cls) -> Dict[str, Any]:
        ...
        return {
            ...
            'course_id': uuid.uuid4(),  # Should be overridden with an actual course ID
            'status': "in_progress",
...
class UserContentProgressFactory(BaseFactory[UserContentProgress]):
    ...
    def _get_defaults(cls) -> Dict[str, Any]:
        ...
        return {
            'user_id': uuid.uuid4(),  # Should be overridden with an actual user ID
            'content_id': uuid.uuid4(),  # Should be overridden with an actual content ID
            'status': "completed",
            'score': 90,
    ...
    def with_user_and_content(
        cls, 
        user: User = None, 
        content: Union[TheoryContent, ExerciseContent, AssessmentContent, InteractiveContent] = None, 
        **kwargs
    ) -> UserContentProgress:
        """
