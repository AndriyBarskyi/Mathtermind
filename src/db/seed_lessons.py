...
from src.db.models import Lesson, TheoryContent, ExerciseContent, AssessmentContent, InteractiveContent
...
def seed_lesson_content():
    ...
    # Find the lesson "Числа та арифметичні операції"
    lesson = db.query(Lesson).filter(Lesson.title.like("%Числа та арифметичні операції%")).first()
    
    if not lesson:
        logger.warning("Lesson 'Числа та арифметичні операції' not found. Skipping content seeding.")
        return
    
    logger.info(f"Found lesson: {lesson.title} with ID: {lesson.id}")
    
    # Check if this lesson already has content by checking the Content table
    from src.db.models import Content
    # Delete existing content for this lesson to ensure a clean seed
    deleted_count = db.query(Content).filter(Content.lesson_id == lesson.id).delete()
    if deleted_count > 0:
        logger.info(f"Deleted {deleted_count} existing content items for lesson '{lesson.title}'.")
    
    # Check if this lesson already has content by checking the Content table
    # from src.db.models import Content # This line is now redundant
    content_count = db.query(Content).filter(Content.lesson_id == lesson.id).count()
    
    if content_count > 0:
        logger.info(f"Lesson already has {content_count} content items. Skipping content seeding.")
        return
...
""",
        ...
        questions={
            "questions": [
                ...
                },
                {
                    ...
                    "options": ["Комутативність", "Асоціативність", "Дистрибутивність", "Всі властивості не виконуються"],
    ...
    interactive = InteractiveContent(
        ...
        description="Вивчення арифметичних операцій за допомогою інтерактивного калькулятора",
        order=4,
        content_type=ContentType.INTERACTIVE,
        interactive_type="tool",
        configuration={
            "tool": "calculator",
            ...
            "challenges": [
                {
                    "id": "c1",
                    "description": "Обчисліть значення виразу: 125 ÷ 5 + 10 × 3",
                    "expected_result": "55",
                    "hint": "Пам'ятайте про порядок операцій!"
                },
                {
                    "id": "c2", 
                    "description": "Знайдіть значення виразу: (18 + 7) × 4 - 15",
                    "expected_result": "85",
                    "hint": "Спочатку виконайте дії в дужках."
                }
            ]
        },
        instructions="""
1. Використовуйте інтерактивний калькулятор для вирішення запропонованих завдань.
2. Ви можете використовувати кнопки на екрані або клавіатуру для введення чисел і операцій.
3. Натисніть '=' для отримання результату.
4. Історія обчислень буде відображатися в правій частині екрану.
5. Спробуйте вирішити запропоновані завдання та перевірте свої відповіді.
""",
        created_at=datetime.now(timezone.utc)
    )
    db.add(interactive)
    ...
    db.commit()
    logger.info(f"Added 4 content items to the lesson: {lesson.title}")
