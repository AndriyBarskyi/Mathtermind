...
from src.db.models import UserContentProgress, Content
...
class UserContentProgressRepository(BaseRepository[UserContentProgress]):
    ...
    def create_progress(self, db: Session, 
                       user_id: uuid.UUID, 
                       ...
                       custom_data: Optional[Dict[str, Any]] = None) -> UserContentProgress:
        """
    ...
    def get_progress(self, db: Session, 
                   user_id: uuid.UUID, 
                   content_id: uuid.UUID) -> Optional[UserContentProgress]:
        """
    ...
    def get_lesson_progress(self, db: Session, 
                          user_id: uuid.UUID, 
                          lesson_id: uuid.UUID) -> List[UserContentProgress]:
        """
    ...
    def update_progress(self, db: Session, 
                      progress_id: uuid.UUID,
    ...
    def mark_as_completed(self, db: Session, 
                        user_id: uuid.UUID, 
                        content_id: uuid.UUID,
                        score: Optional[float] = None,
                        time_spent: Optional[int] = None) -> Optional[UserContentProgress]:
        """
    ...
    def count_completed_content(self, db: Session, 
                              user_id: uuid.UUID, 
                              lesson_id: uuid.UUID) -> int:
        """
    ...
    def get_recent_progress(self, db: Session, 
                          user_id: uuid.UUID, 
                          limit: int = 5) -> List[UserContentProgress]:
        """
        ...
        return db.query(UserContentProgress).filter(
            UserContentProgress.user_id == user_id
        ).order_by(desc(UserContentProgress.last_interaction)).limit(limit).all()
        
    def update_custom_data(self, db: Session, 
                         user_id: uuid.UUID, 
                         content_id: uuid.UUID,
                         custom_data: Dict[str, Any]) -> Optional[UserContentProgress]:
        """
