...
class TagRepository:
    ...
    def create_tag(self, session: Session, name: str, category: Category) -> Optional[Tag]:
        """
    ...
    def add_tag_to_course(
        ...
        try:
            ...
            existing = session.query(CourseTag).filter(
    ...
    def remove_tag_from_course(
        ...
        try:
            ...
            association = session.query(CourseTag).filter(
                CourseTag.tag_id == tag_id,
                CourseTag.course_id == course_id
            ).first()
            
            if not association:
                logging.error(f"Association between tag {tag_id} and course {course_id} not found")
                return False
    ...
    def get_course_tags(self, session: Session, course_id: uuid.UUID) -> List[Tag]:
        ...
        try:
            return session.query(Tag).join(
                CourseTag, Tag.id == CourseTag.tag_id
            ).filter(
                CourseTag.course_id == course_id
            ).all()
        except SQLAlchemyError as e:
    ...
    def get_courses_by_tag(self, session: Session, tag_id: uuid.UUID) -> List[Course]:
        ...
        try:
            return session.query(Course).join(
                CourseTag, Course.id == CourseTag.course_id
            ).filter(
                CourseTag.tag_id == tag_id
            ).all()
        except SQLAlchemyError as e:
