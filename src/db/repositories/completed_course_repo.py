...
class CompletedCourseRepository(BaseRepository[CompletedCourse]):
    ...
    def create_completed_course(self, db: Session, 
                              user_id: uuid.UUID, 
                              course_id: uuid.UUID,
                              ...
                              certificate_id: Optional[uuid.UUID] = None) -> CompletedCourse:
        """
    ...
    def get_user_completed_courses(self, db: Session, user_id: uuid.UUID) -> List[CompletedCourse]:
        """
        ...
        """
        return db.query(CompletedCourse).filter(
            CompletedCourse.user_id == user_id
        ).all()
    
    def is_course_completed(self, db: Session, 
                          user_id: uuid.UUID, 
                          course_id: uuid.UUID) -> bool:
        """
    ...
    def get_course_completion(self, db: Session, 
                            user_id: uuid.UUID, 
                            course_id: uuid.UUID) -> Optional[CompletedCourse]:
        """
    ...
    def add_achievement(self, db: Session, 
                       completion_id: uuid.UUID, 
                       achievement_id: uuid.UUID) -> Optional[CompletedCourse]:
        """
    ...
    def update_certificate(self, db: Session, 
                         completion_id: uuid.UUID, 
                         certificate_id: uuid.UUID) -> Optional[CompletedCourse]:
        """
    ...
    def get_recent_completions(self, db: Session, 
                             user_id: uuid.UUID, 
                             limit: int = 5) -> List[CompletedCourse]:
        """
        ...
        return db.query(CompletedCourse).filter(
            CompletedCourse.user_id == user_id
        ).order_by(CompletedCourse.completed_at.desc()).limit(limit).all() 