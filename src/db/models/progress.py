...
class UserContentProgress(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        Index("uq_user_content_progress_user_content", "user_id", "content_id", unique=True),
    )
...
class ContentState(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    # State type could be "scroll_position", "video_timestamp", "exercise_state", etc.
    state_type: Mapped[str] = mapped_column(String(50), nullable=False)
    # For simple states like scroll position or video timestamp
    numeric_value: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    # For complex states like exercise progress, form data, etc.
    json_value: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSON, nullable=True)
    # Text values for states that are string-based
    ...
    progress: Mapped["Progress"] = relationship("Progress", back_populates="content_states")
    ...
    __table_args__ = (
        ...
        Index("uq_content_state_user_content_type", "user_id", "content_id", "state_type", unique=True),
    )
