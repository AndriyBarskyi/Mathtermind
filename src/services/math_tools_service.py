...
import numpy as np
# Handle scipy import gracefully for test coverage
try:
    from scipy import stats
    SCIPY_AVAILABLE = True
except ImportError:
    # Create dummy stats module for testing
    ...
    class DummyStats:
        ...
        def linregress(self, *args, **kwargs):
            # Return dummy values that match the structure expected by the code
...
class MathToolsService(BaseService):
    ...
    def validate_expression(self, expression: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Check for invalid operators (like consecutive operators)
            ...
            # Try parsing the expression (will catch most syntax errors)
            try:
    ...
    @handle_service_errors(service_name="math_tools")
    def check_answer(self, student_answer: str, correct_answer: str, tolerance: float = 0.001, user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                # If numerical evaluation fails, try string comparison
                # This handles cases like algebraic expressions
    ...
    @handle_service_errors(service_name="math_tools")
    def validate_formula(self, formula: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Try parsing the formula (will catch most syntax errors)
            try:
    ...
    @handle_service_errors(service_name="math_tools")
    def format_expression(self, expression: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for op in ['+', '-', '/', '=']:
                formatted = re.sub(r'([^\s])' + re.escape(op) + r'([^\s])', r'\1 ' + op + r' \2', formatted)
    ...
    @handle_service_errors(service_name="math_tools")
    def prepare_geometry_visualization(self, shape: str, parameters: Dict[str, float], user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if shape == "circle":
                ...
                # Calculate visualization data for a circle
                center_x = parameters.get("center_x", 0)
                center_y = parameters.get("center_y", 0)
                
                # Generate points around the circle for visualization
                ...
                # Calculate visualization data for a rectangle
                center_x = parameters.get("center_x", 0)
                center_y = parameters.get("center_y", 0)
                ...
                vertices = [
                    {"x": center_x - half_width, "y": center_y - half_height},  # Bottom-left
                    {"x": center_x + half_width, "y": center_y - half_height},  # Bottom-right
                    {"x": center_x + half_width, "y": center_y + half_height},  # Top-right
                    {"x": center_x - half_width, "y": center_y + half_height},  # Top-left
                    {"x": center_x - half_width, "y": center_y - half_height}   # Close the shape
                ]
                ...
                # Check for different ways to specify a triangle
                if "vertices" in parameters:
                    # Triangle specified by its vertices
                    ...
                    try:
                        ...
                        points.append({"x": float(vertices[0]["x"]), "y": float(vertices[0]["y"])})
                    ...
                    side1 = math.sqrt((points[0]["x"] - points[1]["x"])**2 + (points[0]["y"] - points[1]["y"])**2)
                    side2 = math.sqrt((points[1]["x"] - points[2]["x"])**2 + (points[1]["y"] - points[2]["y"])**2)
                    side3 = math.sqrt((points[2]["x"] - points[0]["x"])**2 + (points[2]["y"] - points[0]["y"])**2)
                    perimeter = side1 + side2 + side3
                    
                    # Calculate bounds
                    x_values = [p["x"] for p in points[:-1]]  # Exclude the repeated point
                    ...
                    # Triangle specified by side lengths
                    ...
                    area = math.sqrt(s * (s - side_a) * (s - side_b) * (s - side_c))  # Heron's formula
                    
                    # Calculate vertices (place the triangle in a logical position)
                    # For simplicity, we'll place the first vertex at the origin,
                    # the second vertex at (side_a, 0),
                    # and calculate the third vertex using the law of cosines
                    ...
                    vertices = [
                        ...
                        {"x": side_c * math.cos(angle), "y": side_c * math.sin(angle)}  # Third vertex
                    ]
            ...
            logger.error(f"Error preparing geometry visualization for {shape}: {str(e)}")
            return {
                "shape": shape,
                "error": str(e),
                "success": False
    ...
    @handle_service_errors(service_name="math_tools")
    def prepare_statistics_visualization(self, data: List[float], visualization_type: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if user_id:
            self.tracking_service.track_tool_usage(
                ...
                data={"visualization_type": visualization_type, "data_points_count": len(data)}
            )
        ...
        try:
            ...
            # Convert all data to float to ensure consistency
            try:
            ...
            # Basic statistics calculated for all visualization types
            basic_stats = {
                "count": len(data),
                "min": min(data),
                "max": max(data),
            ...
            if visualization_type == "histogram":
                ...
                # Prepare bin data for visualization
                bins = []
                ...
                # Calculate quartiles, min, max for boxplot
                ...
                # Identify outliers
                outliers = [float(x) for x in data if x < lower_whisker or x > upper_whisker]
                ...
                # For scatter plots, we need paired data
                # If only one series is provided, we'll use indices as x-coordinates
                points = [{"x": float(i), "y": float(val)} for i, val in enumerate(data)]
    ...
    @handle_service_errors(service_name="math_tools")
    def prepare_function_graph_data(self, function_expression: str, x_range: Tuple[float, float], 
                                 num_points: int = 100, user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if user_id:
            self.tracking_service.track_tool_usage(
                ...
                data={"function": function_expression, "x_range": x_range, "num_points": num_points}
            )
        ...
        try:
            ...
            # Preprocess the function expression
            preprocessed_expression = self._preprocess_expression(function_expression)
            
            # Replace 'x' with a placeholder that won't conflict with other variables
            ...
            for x in x_values:
                try:
                    # Replace the placeholder with the current x value
                    expr = expression_template.format(x=x)
                    y = self._evaluate_parsed_expression(expr)
                    
                    # Check for very large values or NaN/Inf
            ...
            # Add padding to the y range (10% on each side)
            y_padding = (y_max - y_min) * 0.1 if y_max > y_min else 1.0
            y_range = (y_min - y_padding, y_max + y_padding)
            
            # Handle the case where y_min and y_max are very close or equal
            ...
            logger.error(f"Error preparing graph data for function '{function_expression}': {str(e)}")
            return {
                "function": function_expression,
                "x_range": x_range,
                "data_points": [],
    ...
    def _normalize_expression(self, expression: str) -> str:
        ...
        """
        # Basic normalization without evaluation
        normalized = expression.replace(' ', '').replace('^', '**')
        
        # Don't try to evaluate the expression, just return the normalized form
        return normalized
    ...
    def _tokenize_expression(self, expression: str) -> List[str]:
        ...
        while i < len(expression):
            ...
            # Handle operators
            if char in OPERATORS or (char == '*' and i + 1 < len(expression) and expression[i + 1] == '*'):
            ...
            # Handle functions and variables (which could be multiple characters)
            if char.isalpha():
                j = i
            ...
            # Skip other characters (should not get here if the expression is valid)
            i += 1
    ...
    def _has_invalid_functions(self, expression: str) -> bool:
        ...
        """
        # Find all function-like patterns and check if they are valid
    ...
    # Helper method needed for checking answers
    ...
    def _evaluate_tokens(self, tokens: List[str]) -> float:
        ...
        while i < len(tokens):
            ...
            # If token is a function, push onto operator stack
            ...
            # If token is left parenthesis, push onto operator stack
            elif token == '(':
                operator_stack.append(token)
            
            # If token is right parenthesis, process until matching left parenthesis
            elif token == ')':
