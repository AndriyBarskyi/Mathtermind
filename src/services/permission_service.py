...
class PermissionService:
    ...
    def user_has_permission(user: Dict[str, Any], permission: Union[Permission, str]) -> bool:
        ...
        try:
            ...
            logger.debug(f"User {user_id} with role {user_role} {'has' if has_permission else 'does not have'} "
            ...
            report_error(e, operation="user_has_permission", user_id=user_id, permission=str(permission))
            return False
    ...
    def user_has_permissions(user: Dict[str, Any], permissions: List[Union[Permission, str]]) -> bool:
        ...
        if not user or not permissions:
            logger.warning("Attempted permissions check with empty user or permissions list")
            return False
            
        user_id = user.get('id', 'unknown')
        logger.debug(f"Checking if user {user_id} has all permissions: {permissions}")
        
        result = all(PermissionService.user_has_permission(user, perm) for perm in permissions)
        logger.debug(f"User {user_id} {'has' if result else 'does not have'} all required permissions")
        return result
    ...
    def user_has_any_permission(user: Dict[str, Any], permissions: List[Union[Permission, str]]) -> bool:
        ...
        user_id = user.get('id', 'unknown')
        logger.debug(f"Checking if user {user_id} has any permissions from: {permissions}")
        
        result = any(PermissionService.user_has_permission(user, perm) for perm in permissions)
        logger.debug(f"User {user_id} {'has' if result else 'does not have'} any of the permissions")
        return result
    ...
    def is_resource_owner(user: Dict[str, Any], resource: Dict[str, Any]) -> bool:
        ...
        for field in ['user_id', 'author_id', 'owner_id', 'created_by']:
            ...
            if resource_owner_id and resource_owner_id == user_id:
                logger.debug(f"User {user_id} is owner of resource {resource_id} (field: {field})")
                return True
    ...
    def has_access_to_resource(user: Dict[str, Any], resource: Dict[str, Any], 
        ...
        user_id = user.get('id', 'unknown')
        resource_id = resource.get('id', 'unknown')
        logger.debug(f"Checking if user {user_id} has access to resource {resource_id} "
        ...
        if PermissionService.user_has_permission(user, required_permission):
            logger.debug(f"User {user_id} has permission {required_permission} for resource {resource_id}")
            return True
