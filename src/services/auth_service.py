...
class AuthService(BaseService):
    ...
    def register(self, username: str, email: str, password: str,
        ...
        try:
            ...
            if not is_valid:
                logger.warning(f"Registration failed: Password strength validation failed")
    ...
    def logout(self, session_token: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(f"Logout failed: Invalid session token: {session_token[:8]}...")
    ...
    def change_password(self, user_id: str, current_password: str, new_password: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if not is_valid:
                logger.warning(f"Password change failed: New password fails strength validation")
            ...
            try:
                ...
                logger.warning(f"Password change failed: Invalid user ID format: {user_id}")
    ...
    def request_password_reset(self, email: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if not user:
                ...
                # Just log it and return success with no token
                logger.warning(f"Password reset failed: User not found with email: {email}")
                return (True, None)
    ...
    def reset_password(self, token: str, new_password: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            is_valid, errors = validate_password_strength(new_password)
            if not is_valid:
                logger.warning(f"Password reset failed: New password fails strength validation")
                return (False, "New password does not meet strength requirements")
            ...
            # Convert expires_at to the appropriate type before comparison
            ...
            with self.transaction() as session:
                user = user_repo.get_by_id(session, user_uuid)
                if not user:
                    logger.warning(f"Password reset failed: User not found with ID: {user_id}")
                    return (False, "User not found")
